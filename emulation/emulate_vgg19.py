...
# Model definition of VGG19 32-bit
class VGG19_32bit(torch.nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5):
        super(VGG19_32bit, self).__init__()
    ...
    def forward(self, x):
        ...
        x = self.conv14(x)
        x = self.conv14_nonlinear(x)
        x = self.conv15(x)
        x = self.conv15_nonlinear(x)
        x = self.conv16(x)
        x = self.conv16_nonlinear(x)
...
# Model definition of VGG19 8-bit
class VGG19_8bit(torch.nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5):
        super(VGG19_8bit, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_nonlinear = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.conv2_nonlinear = nn.Sequential(nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        x = self.conv14(x)
        x = self.adjust_conv_bias(x, scaling_factor, self.conv14.bias)
        scaling_factor, x = self.convert_float8_inter_layer(x)
        x = self.conv14_nonlinear(x)

        x = self.conv15(x)
        x = self.adjust_conv_bias(x, scaling_factor, self.conv15.bias)
        scaling_factor, x = self.convert_float8_inter_layer(x)
        x = self.conv15_nonlinear(x)

        x = self.conv16(x)
        x = self.adjust_conv_bias(x, scaling_factor, self.conv16.bias)
...
# Model definition of VGG19 8-bit with noise
class VGG19_8bit_noise(torch.nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5):
        super(VGG19_8bit_noise, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_nonlinear = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.conv2_nonlinear = nn.Sequential(nn.ReLU(inplace=True),
...
# Load model parameter for VGG19 32-bit model
def load_vgg19_model_32bit():
    model = VGG19_32bit()
    original_model = torch.load("models_weight/vgg19.pth")
    model_32bit = models.vgg19()
    ...
    model.conv8.weight = model_32bit.features[16].weight
    model.conv8.bias = model_32bit.features[16].bias
    ...
    model.conv11.weight = model_32bit.features[23].weight
    model.conv11.bias = model_32bit.features[23].bias

    model.conv12.weight = model_32bit.features[25].weight
    model.conv12.bias = model_32bit.features[25].bias

    model.conv13.weight = model_32bit.features[28].weight
    model.conv13.bias = model_32bit.features[28].bias

    model.conv14.weight = model_32bit.features[30].weight
    model.conv14.bias = model_32bit.features[30].bias

    model.conv15.weight = model_32bit.features[32].weight
    model.conv15.bias = model_32bit.features[32].bias

    model.conv16.weight = model_32bit.features[34].weight
    model.conv16.bias = model_32bit.features[34].bias
...
# Load model parameter for VGG19 8-bit model
def load_vgg19_model_8bit():
    model_path = "models_weight/vgg19.pth"
    model = VGG19_8bit()
    original_model = torch.load(model_path)
    model_32bit = models.vgg19()
    model_32bit.load_state_dict(original_model)
    ...
    model.conv8.weight = convert_float8_weight(model_32bit.features[16].weight, scaling_factor)
    model.conv8.bias = convert_float8_bias(model_32bit.features[16].bias, scaling_factor)
    ...
    model.conv11.weight = convert_float8_weight(model_32bit.features[23].weight, scaling_factor)
    model.conv11.bias = convert_float8_bias(model_32bit.features[23].bias, scaling_factor)

    model.conv12.weight = convert_float8_weight(model_32bit.features[25].weight, scaling_factor)
    model.conv12.bias = convert_float8_bias(model_32bit.features[25].bias, scaling_factor)
    ...
    model.conv14.weight = convert_float8_weight(model_32bit.features[30].weight, scaling_factor)
    model.conv14.bias = convert_float8_bias(model_32bit.features[30].bias, scaling_factor)

    model.conv15.weight = convert_float8_weight(model_32bit.features[32].weight, scaling_factor)
    model.conv15.bias = convert_float8_bias(model_32bit.features[32].bias, scaling_factor)

    model.conv16.weight = convert_float8_weight(model_32bit.features[34].weight, scaling_factor)
    model.conv16.bias = convert_float8_bias(model_32bit.features[34].bias, scaling_factor)
...
def load_vgg19_model_8bit_noise():
    model_path = "models_weight/vgg19.pth"
    model = VGG19_8bit_noise()
    original_model = torch.load(model_path)
    model_32bit = models.vgg19()
    model_32bit.load_state_dict(original_model)
    scaling_factor = find_weight_scaling_factor(model_path, model_32bit)
...
if __name__ == '__main__':
    ...
    print("Emulate VGG19 32-bit...")
    file_name = "results/vgg19_digital_32bit.txt"
    create_file(file_name)
    for i in range(experiment_time):
        model = load_vgg19_model_32bit().to(device)
    ...
    print("Emulate VGG19 8-bit...")
    file_name = "results/vgg19_digital_8bit.txt"
    create_file(file_name)
    for i in range(experiment_time):
        model = load_vgg19_model_8bit().to(device)
    ...
    print("Emulate VGG19 8-bit with photonic noise...")
    file_name = "results/vgg19_lightning.txt"
    create_file(file_name)
    for i in range(experiment_time):
        model = load_vgg19_model_8bit_noise().to(device)
