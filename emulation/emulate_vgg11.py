...
# Model definition of VGG11 32-bit
class VGG11_32bit(torch.nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5):
        super(VGG11_32bit, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_nonlinear = nn.Sequential(nn.ReLU(inplace=True),
        ...
        self.conv6_nonlinear = nn.Sequential(nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2))
...
# Model definition of VGG11 8-bit
class VGG11_8bit(torch.nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5):
        super(VGG11_8bit, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_nonlinear = nn.Sequential(nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
...
# Model definition of VGG11 8-bit with noise
class VGG11_8bit_noise(torch.nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5):
        super(VGG11_8bit_noise, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_nonlinear = nn.Sequential(nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
...
# Load model parameter for VGG11 32-bit model
def load_vgg11_model_32bit():
    model = VGG11_32bit()
    original_model = torch.load("models_weight/vgg11.pth")
    model_32bit = models.vgg11()
    ...
    model.conv5.weight = model_32bit.features[11].weight
    model.conv5.bias = model_32bit.features[11].bias

    model.conv6.weight = model_32bit.features[13].weight
    model.conv6.bias = model_32bit.features[13].bias

    model.conv7.weight = model_32bit.features[16].weight
    model.conv7.bias = model_32bit.features[16].bias

    model.conv8.weight = model_32bit.features[18].weight
    model.conv8.bias = model_32bit.features[18].bias
...
# Load model parameter for VGG11 8-bit model
def load_vgg11_model_8bit():
    model_path = "models_weight/vgg11.pth"
    model = VGG11_8bit()
    original_model = torch.load(model_path)
    model_32bit = models.vgg11()
    model_32bit.load_state_dict(original_model)
    ...
    model.conv5.weight = convert_float8_weight(model_32bit.features[11].weight, scaling_factor)
    model.conv5.bias = convert_float8_bias(model_32bit.features[11].bias, scaling_factor)

    model.conv6.weight = convert_float8_weight(model_32bit.features[13].weight, scaling_factor)
    model.conv6.bias = convert_float8_bias(model_32bit.features[13].bias, scaling_factor)

    model.conv7.weight = convert_float8_weight(model_32bit.features[16].weight, scaling_factor)
    model.conv7.bias = convert_float8_bias(model_32bit.features[16].bias, scaling_factor)

    model.conv8.weight = convert_float8_weight(model_32bit.features[18].weight, scaling_factor)
    model.conv8.bias = convert_float8_bias(model_32bit.features[18].bias, scaling_factor)
...
# Load model parameter for VGG11 8-bit model with noise
def load_vgg11_model_8bit_noise():
    model_path = "models_weight/vgg11.pth"
    model = VGG11_8bit_noise()
    original_model = torch.load(model_path)
    model_32bit = models.vgg11()
    model_32bit.load_state_dict(original_model)
    scaling_factor = find_weight_scaling_factor(model_path, model_32bit)
...
if __name__ == '__main__':
    ...
    print("Emulate VGG11 32-bit...")
    file_name = "results/vgg11_digital_32bit.txt"
    create_file(file_name)
    for i in range(experiment_time):
        model = load_vgg11_model_32bit().to(device)
    ...
    print("Emulate VGG11 8-bit...")
    file_name = "results/vgg11_digital_8bit.txt"
    create_file(file_name)
    for i in range(experiment_time):
        model = load_vgg11_model_8bit().to(device)
    ...
    print("Emulate VGG11 8-bit with photonic noise...")
    file_name = "results/vgg11_lightning.txt"
    create_file(file_name)
    for i in range(experiment_time):
        model = load_vgg11_model_8bit_noise().to(device)
